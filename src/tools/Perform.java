...
/**
 * Sketch: CLI to render an MP3 from MEI + MPM via meico, optionally filtering to given MEI ids
 * and, if filtered, shifting expressive onsets to start at the first selected note.
 *
 * Build/run (example):
 *   javac -cp .:path/to/meico.jar src/tools/MeiMpmToMp3.java
 *   java -cp .:path/to/meico.jar tools.MeiMpmToMp3 \
 *       --mei path/to/score.mei \
 *       --mpm path/to/performance.mpm \
 *       --out out.mp3 \
 *       --ids n1,n2,n3 \
 *       --ppq 720 \
 ...
 *
 * Arguments:
 *   --mei <file>            Path to input MEI file (required)
 *   --mpm <file>            Path to MPM encoding (required)
 *   --out <file>            Path to output MP3 file (required; .mp3)
 *   --ids <csv>             Optional CSV of MEI xml:ids to keep; all other notes are filtered out
 *   --ppq <int>             Optional PPQ for MEI→MSM conversion (default 720)
 *   --movement <int>        Optional movement index to render (default 0)
 *   --ignore-expansions     Optional: MEI→MSM without processing expansion elements
 *   --use-channel-10        Optional: allow MIDI channel 10 (drums); default is to not use it
 *   --soundfont <file>      Optional SF2/SF3/DLS soundbank; default system synthesizer bank
 *
 * Behavior:
 ...
 *   - Synthesizes audio and writes MP3.
 *
 * Adjust attribute and method names as needed to match your meico version.
 */
public class Perform {

    public static void main(String[] args) throws Exception {
        Map<String, String> flags = parseArgs(args);
        if (!flags.containsKey("mei")
            || !flags.containsKey("mpm")
            || !flags.containsKey("out")
            || !flags.containsKey("ranges")
        ) {
            printUsageAndExit("Missing required arguments.");
        }

        File meiFile = new File(flags.get("mei"));
        File mpmFile = new File(flags.get("mpm"));
        File outFile = new File(flags.get("out"));
        File rangesFile = new File(flags.get("ranges"));

        if (!meiFile.isFile()) printUsageAndExit("MEI file not found: " + meiFile);
        if (!mpmFile.isFile()) printUsageAndExit("MPM file not found: " + mpmFile);

        int ppq = parseIntOrDefault(flags.get("ppq"), 720);
        int movementIndex = parseIntOrDefault(flags.get("movement"), 0);
        boolean ignoreExpansions = flags.containsKey("ignore-expansions");
        boolean dontUseChannel10 = !flags.containsKey("use-channel-10"); // default: do not use channel 10

        Set<String> keepIds = Collections.emptySet();
        if (flags.containsKey("ids")) {
            keepIds = Arrays.stream(flags.get("ids").split(","))
                    .map(String::trim)
                    .filter(s -> !s.isEmpty())
        ...
        System.out.println("Converting MEI to MSM/MPM (ppq=" + ppq + ", ignoreExpansions=" + ignoreExpansions + ", dontUseChannel10=" + dontUseChannel10 + ")");
        KeyValue<List<Msm>, List<Mpm>> msmMpm = mei.exportMsmMpm(ppq, dontUseChannel10, ignoreExpansions, true);
        ...
        midi.writeMidi(outFile.getAbsolutePath() + ".mid");
        System.out.println("Wrote MIDI: " + outFile.getAbsolutePath());

        // 7) Synthesize audio and write MP3
        //Soundbank sb = null;
        //if (flags.containsKey("soundfont")) {
        //    File sf = new File(flags.get("soundfont"));
        //    if (!sf.isFile()) {
        //        throw new IllegalArgumentException("SoundFont not found: " + sf);
        //    }
        //    sb = MidiSystem.getSoundbank(sf);
        //    System.out.println("Using SoundFont: " + sf.getAbsolutePath());
        //} else {
        //    System.out.println("Using default system synthesizer soundbank.");
        //}

        // System.out.println("Rendering audio ...");
        // Audio audio = midi.exportAudio(sb);

        //System.out.println("Writing MP3: " + outFile.getAbsolutePath());
        // Adjust this to your meico Audio API:
        // e.g., audio.exportMp3(outFile) or audio.saveAsMp3(outFile)
        // audio.writeMp3(outFile.getAbsolutePath());
